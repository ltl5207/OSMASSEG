...
def load_and_save_results(destdir,
    ...
    #opt.gpu_ids = 0
    opt.dataroot = '/home/pc/diskB/liTianLin/triple/datasets/pddca_testset9'
    opt.name = name
    opt.CUT_mode = 'CUT'
    opt.phase = 'test'
...
def compute_dice_given_dir(image_dir,gtdir):
    ...
    parser.add_argument('--dataset', type=str, default='pddca_syn36')
    ...
    def inference(args, model, test_save_path=None, gtdir='', split="gantest"):
        ...
        hist_matrix,dice,hist_matrix_groupby_individual,stdlist3, stdlist6, stdlistmean = cauculate_confusion_matrix_dice(args, test_save_path, gtdir, 7, model)
        for i in range(0, args.num_classes):
            print('Dice of %d class is %f' % (i, dice[i]))
        #print('std is:' + str(stdlist))
        performance = np.mean(dice[1:args.num_classes])
        return dice[1:args.num_classes], performance, stdlist3, stdlist6, stdlistmean
    ...
    #要存，看结果
    if args.is_savenii:
        args.test_save_dir = '../predictions_pddcav1'
        test_save_path = os.path.join(args.test_save_dir, args.exp, snapshot_name)
        os.makedirs(test_save_path, exist_ok=True)
    else:
        test_save_path = None
    dice,performance,stdlist3,stdlist6,stdlistmean = inference(args, net, test_save_path, gtdir=gtdir)
    return dice,performance,stdlist3,stdlist6,stdlistmean
...
if __name__ == '__main__':
    destdir = '/home/pc/diskB/liTianLin/trans-unet/ganresult/plguide_pddca'
    epochlist = np.linspace(1,400,400,endpoint=True, dtype=int)
    name = 'real2syn_pddca'
    checkpdir = '/home/pc/diskB/liTianLin/triple/checkpoints/'
    gtdir = '/home/pc/diskB/liTianLin/trans-unet/data/pddca_testset9/Seg'
    performance_dict = {}
    epochlist=[336]
    for _,epoch in enumerate(epochlist):
        ...
        dice, performance, stdlist3, stdlist6, stdlistmean = compute_dice_given_dir(result_dir, gtdir=gtdir)
        performance_dict['{}'.format(epoch)]=(dice, performance, stdlist3, stdlist6, stdlistmean)

    dict2csv(performance_dict, 'performances_per_model_pddcav4_on_tu79.csv')
