...
from trans_unet import create_and_load_TU,freeze
import torchvision.transforms as transforms

def get_transform_for_pl():
...
class CUTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        #parser.add_argument('--lambda_Seg', type=float, default=0.3, help='weight for seg loss')
        parser.add_argument('--lambda_SRC_pl', type=float, default=0.01, help='weight for SRC loss for pseudo-label')
        ...
        parser.add_argument('--TU', type=str, default='TU_acdc_10percent_ref_ep109.pth')#'./TU_acdc_from099_ep39_90rot.pth')
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            self.netTU = create_and_load_TU(snapshot_path=opt.TU)
            freeze(self.netTU)
        ...
        if self.isTrain:
            ...
            self.transformforpl = get_transform_for_pl()
    ...
    def set_input(self, input):
        ...
        #m
            ...
            #self.seg_paths = input['Aseg_paths']
    ...
    def forward(self):
        ...
        if self.isTrain == True:
            real_A_zoom = torch.nn.functional.interpolate(self.real_A, (224,224),mode='bicubic')
            self.pseudo_label = self.netTU(real_A_zoom)
            #softmax是为了转成概率的形式
            self.pseudo_label = torch.argmax(torch.softmax(self.pseudo_label, dim=1), dim=1).squeeze(0)
            #直接得位置不好转成序列id，应该先插值再转位置
            self.pl_location = torch.nonzero(self.pseudo_label)
            #插值成5层特征图尺寸，且插值不能接收长整型格式数据
            #注意，即使用了nearest插值，输出也是浮点型，要转回int64
            self.pseudo_labelf1 = torch.nn.functional.interpolate(torch.unsqueeze(self.pseudo_label.float(),1), (262,262), mode='nearest').long()
            self.pseudo_labelf2 = torch.nn.functional.interpolate(torch.unsqueeze(self.pseudo_label.float(),1), (256,256), mode='nearest').long()
            self.pseudo_labelf3 = torch.nn.functional.interpolate(torch.unsqueeze(self.pseudo_label.float(),1), (128,128), mode='nearest').long()
            self.pseudo_labelf45 = torch.nn.functional.interpolate(torch.unsqueeze(self.pseudo_label.float(),1), (64, 64), mode='nearest').long()
            temp = torch.nn.functional.interpolate(torch.unsqueeze(self.pseudo_label.float(),1),
                                                    (self.real_A.shape[2],self.real_A.shape[3]), mode='nearest').long()
            self.pseudo_labelR = temp.repeat(1,3,1,1)
        #pass
    ...
    def compute_sampleids_given_pl(self):
        bs = self.opt.batch_size
        pl1 = self.pseudo_labelf1       #对应特征层1，4, 3, 262, 262
        pl2 = self.pseudo_labelf2       #对应2，4, 128, 256, 256
        pl3 = self.pseudo_labelf3       #对应3，4, 256, 128, 128
        pl45 = self.pseudo_labelf45     #对应45，4, 256, 64, 64

        #先取，再判断够不够num_patches
        num_patches = self.opt.num_patches
        map1 = torch.squeeze(pl1.permute(0,2,3,1).flatten(1,2), 2)  #4,68644
        map2 = torch.squeeze(pl2.permute(0,2,3,1).flatten(1,2), 2)    #4,65536
        map3 = torch.squeeze(pl3.permute(0,2,3,1).flatten(1,2), 2)    #4,16384
        map45 = torch.squeeze(pl45.permute(0,2,3,1).flatten(1,2), 2)  #4,4096
        idlist = []
        for _,map in enumerate([map1,map2,map3,map45]):
            #templist = []
            templist = torch.zeros(bs,num_patches).to(self.device).long()
            for i in range(bs):
                map_id_i = torch.nonzero(map[i])
                if(len(map_id_i) >= num_patches):
                    map_id_i = torch.squeeze(map_id_i[0:num_patches])
                elif(len(map_id_i)==0):
                    #以防还有伪标签全是背景的
                    map_id_i = torch.randperm(len(map[i]), device=self.device)
                    map_id_i = map_id_i[:num_patches]
                else:
                    setdiff = set(np.arange(0,len(map[i]))) - set(torch.squeeze(map_id_i,1).tolist())
                    setresult = random.sample(setdiff, num_patches-len(map_id_i))
                    map_id_i = torch.cat([torch.squeeze(map_id_i,1),torch.tensor(list(setresult)).to(self.device)], 0)
                #templist.append(map_id_i)
                templist[i] = map_id_i
            idlist.append(templist)
        #map4 and map5 have the same size, so use the same psudolabel ids
    ...
    def compute_G_loss(self):
        ...
        ids = self.compute_sampleids_given_pl()

        fake_B_pool, _ = self.netF(fake_B_feat, self.opt.num_patches, ids, guide=True)
        #fordbg
        #fake_B_pool, sample_ids = self.netF.forward(fake_B_feat, self.opt.num_patches, None)
        #real_A_pool, _ = self.netF.forward(real_A_feat, self.opt.num_patches, sample_ids)

        real_A_pool, _ = self.netF(real_A_feat, self.opt.num_patches, ids, guide=True)
        ...
        if self.opt.dce_idt:
            ...
            idt_B_pool, sample_ids_B = self.netF(idt_B_feat, self.opt.num_patches, None, guide=False)
            real_B_pool, _ = self.netF(real_B_feat, self.opt.num_patches, sample_ids_B, guide=False)
        ...
        ## Relation loss for label
        label_feat = self.netG(self.transformforpl(self.pseudo_labelR.float()/255),self.nce_layers, encode_only=True)
        label_pool,_ = self.netF(label_feat, self.opt.num_patches, ids, guide=True)
        self.loss_SRC_L, weightL = self.calculate_R_loss(real_A_pool, label_pool, epoch=self.train_epoch)
        self.loss_SRC_L = self.loss_SRC_L * self.opt.lambda_SRC_pl
        ## HDCE
        ...
        #self.loss_G = self.loss_G_GAN + loss_HDCE_both + self.loss_SRC + 0.4*self.loss_seg
        self.loss_G = self.loss_G_GAN + loss_HDCE_both + self.loss_SRC + self.loss_SRC_L
        return self.loss_G
    ...
    def calculate_HDCE_loss(self, src, tgt, weight=None):
        ...
        for f_q, f_k, crit, nce_layer, w in zip(feat_q_pool, feat_k_pool, self.criterionHDCE, self.nce_layers, weight):
            if self.opt.no_Hneg:
                w = None
            #test
            #loss = crit.forward(f_q,f_k,w) * self.opt.lambda_HDCE
    ...
    def calculate_R_loss(self, src, tgt, only_weight=False, epoch=None):
        ...
        for f_q, f_k, crit, nce_layer in zip(feat_q_pool, feat_k_pool, self.criterionR, self.nce_layers):
            #test
            #loss_SRC, weight = crit.forward(f_q, f_k, only_weight, epoch)
