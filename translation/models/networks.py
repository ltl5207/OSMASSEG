...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, guide=False):

        '''
        guide为True时，patch_ids一定不能为None
        '''
        ...
        if guide==True:
            for feat_id, feat in enumerate(feats):
                ...
                if num_patches > 0:
                    #这个时候要取出来这个特征层对应的一个batch的batchid，不再是长1024的列表，是bs x 1024
                    #batch_patch_id: bs(4)x1024
                    #feat_reshape: 4 x 68644 x 3
                    batch_patch_id = patch_ids[feat_id]

                    #list里含tensor的情况，list转tensor直接torch.tensor不可以
                    #torch.tensor([item.cpu().detach().numpy() for item in batch_patch_id]).cuda()
                    #从源头上改成4x1024的tensor了，不需要了，但是仍然值得学习

                    #直接整体索引，不循环，bsz不为1的时候，这样比循环快很多很多
                    rowindex = torch.arange(start=0, end=bs, step=1).unsqueeze(1).long()
                    x_sample = feat_reshape[rowindex, batch_patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1])

                    #for i in range(bs):
                        #feat_reshape[:, patch_id, :].flatten(0, 1)  # reshape(-1, x.shape[1])
                else:
                ...
                return_ids.append(batch_patch_id)
            ...
            for feat_id, feat in enumerate(feats):
                ...
                if num_patches > 0:
                    if patch_ids is not None:
                        ...
                        #按给定采样数截取序列前面部分，如果采样数大于序列长就全部使用
...
class PatchSampleF_backup(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(PatchSampleF_backup, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
