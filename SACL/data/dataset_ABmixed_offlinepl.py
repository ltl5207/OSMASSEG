    ...
    def __getitem__(self, index):
        if self.split == 'train':
            ...
            if domain_flag == 'A':
                ...
                #im_G_aW, angle = weak_aug(im_aG)
                im_aW, angle = weak_aug(im_arr_zoomed)

                # im_aS, d_field = strong_aug(input_pic=im_aG, alpha=im_aG.shape[1], sigma=im_aG.shape[1]*0.08)
                d_field = strong_aug(input_pic=im_aG, alpha=im_aG.shape[1], sigma=im_aG.shape[1] * 0.08)
                d_field = util.map2sample(torch.from_numpy(d_field))

                im_G_aS = F.grid_sample(torch.from_numpy(im_aG).unsqueeze(0).unsqueeze(0), d_field.unsqueeze(0),
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() > 0.5:
            im_aW, im_aS, label = random_rot_flip(im_aW, im_aS, label)
        elif random.random() <= 0.5:
            im_aW, im_aS, label = random_rotate(im_aW, im_aS, label)
        '''
...
def random_rot_flip(image1, image2, label):
...
def random_rotate(image1, image2, label):
