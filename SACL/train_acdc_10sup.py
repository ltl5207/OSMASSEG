...
def load_and_freeze_TU(freeze_flag=False,snapshot_path='./TU_acdc_10percent_ref_ep69.pth',num_classes=4):
    TU = create_and_load_TU(snapshot_path=snapshot_path,num_classes=num_classes)
    if freeze_flag:
        freeze(TU)
    return TU

def load_and_freeze_G(pth_path='acdc_netG_10perv4_25.pth',device=torch.device('cpu')):
...
# command:  CUDA_VISIBLE_DEVICES=0 python ./train_acdc_10sup.py --root_path ./datasets/ABmixed_acdc_10percentsup --list_dir ./lists/lists_acdc_10percentsup --num_classes 4
# command:  CUDA_VISIBLE_DEVICES=0 python ./train_acdc_10sup.py --root_path ./datasets/ABmixed_acdc_10percentsup_doubledeform --list_dir ./lists/lists_acdc_10percentsup_doubledeform --num_classes 4
# continue:  python ./train_acdc_v1.py --root_path ./datasets/ABmixed_acdc_nocrop --list_dir ./lists/lists_acdc_nocrop --num_classes 4 --continue_train --epoch_count 148
# CUDA_VISIBLE_DEVICES=0 python ./train_acdc_10sup.py --root_path ./datasets/ABmixed_acdc_10percentsup --list_dir ./lists/lists_acdc_10percentsup --num_classes 4 --continue_train --epoch_count 149
if __name__ == '__main__':
    ...
    snapshot_path1 = './checkpoints/TU_acdc_10persup/TU1'
    snapshot_path2 = './checkpoints/TU_acdc_10persup/TU2'
    snapshot_path1 = './checkpoints/TU_acdc_10persup_v2/TU1'
    snapshot_path2 = './checkpoints/TU_acdc_10persup_v2/TU2'
    ...
    if opt.continue_train == False:
        ...
        TU1 = load_and_freeze_TU(snapshot_path=os.path.join(snapshot_path1, 'epoch_' + str(149) + '.pth'),num_classes=num_classes)
        TU2 = load_and_freeze_TU(snapshot_path=os.path.join(snapshot_path2, 'epoch_' + str(opt.epoch_count) + '.pth'),num_classes=num_classes)
        TU = load_and_freeze_TU(freeze_flag=True,num_classes=num_classes)

    netG_A2B = load_and_freeze_G(pth_path='./acdc_netG_10perv4_25.pth',device=device)
    ...
    db_train = dataset_ABmixed(opt, segopt='Seg', transform=transforms.Compose([RandomGenerator(output_size=[opt.img_size, opt.img_size])]),
    ...
    ce_loss_sup1 = CrossEntropyLoss()#weight=torch.tensor([1., 4., 4., 2.]).to(device))
    ce_loss_sup2 = CrossEntropyLoss()#weight=torch.tensor([1., 4., 4., 2.]).to(device))
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_dice_sup1 = dice_loss1(outputs_TU1, label_batch_deform.squeeze().long(), softmax=True)
            loss_dice_sup2 = dice_loss2(outputs_TU2_reverse, label_batch, softmax=True)
            ...
            logging.info('iteration %d : loss : %f, loss_ce: %f, loss_ce_sup1: %f , loss_ce_sup2: %f, loss_dicesup1: %f, loss_dicesup2: %f, loss_R: %f, loss_dce: %f' \
                         % (iter_num, total_L.item(), loss_ce.item(), loss_ce_sup1.item(), loss_ce_sup2.item(), loss_dice_sup1.item(), loss_dice_sup2.item(),loss_SRC.item(), loss_HDCE.item()))
