...
parser.add_argument('--dataset', type=str,
                    default='pddca_testset9', help='experiment_name')
...
def new_inference(args, model, predict_path=None, gt_dir=None):
    ...
    print(D)
    A = np.zeros((d_num, 7))
    MSD = np.zeros((d_num, 7))
    AHD = np.zeros((d_num, 7))
    fz = np.zeros((d_num, 7))
    fm = np.zeros((d_num, 7))
    ####
    pred_vol, mask_vol = [], []
    ...
    for filename in pred_mask_vol:
        ...
        for i in range(1, 7):
            ...
            for j in range(tmp.shape[0]):
                ...
                A_.append([tmp[j][0]*1.08, tmp[j][1]*1.08, tmp[j][2]*1.08])
            tmp = np.argwhere(raw_ann_cp == i)
            for j in range(tmp.shape[0]):
                #B_.append([tmp[j][0] * 3, tmp[j][1] * 1.08, tmp[j][2] * 1.08])
                B_.append([tmp[j][0]*1.08, tmp[j][1]*1.08, tmp[j][2]*1.08])
            if len(A_) == 0:
                print(cnt, i)
            else:
    ...
    ahd_mean_14 = np.mean(AHD_details[:, 0])
    ahd_mean_36 = np.mean(AHD_details[:, 1])
    ahd_mean_25 = np.mean(AHD_details[:, 2])
    ahd_std_14 = np.std(AHD_details[:, 0])
    ahd_std_36 = np.std(AHD_details[:, 1])
    ahd_std_25 = np.std(AHD_details[:, 2])
    ahd_std_all = np.std(np.mean(AHD_details, axis=1))
    ahd_std_2536 = np.std(np.mean(AHD_details[:, 1:], axis=1))
    msd_mean_14 = np.mean(MSD_details[:, 0])
    msd_mean_36 = np.mean(MSD_details[:, 1])
    msd_mean_25 = np.mean(MSD_details[:, 2])
    msd_std_14 = np.std(MSD_details[:, 0])
    msd_std_36 = np.std(MSD_details[:, 1])
    msd_std_25 = np.std(MSD_details[:, 2])
    msd_std_all = np.std(np.mean(MSD_details, axis=1))
    msd_std_2536 = np.std(np.mean(MSD_details[:, 1:], axis=1))
    result_dict = {}
    result_dict['ahd14'] = [ahd_mean_14, ahd_std_14]
    result_dict['ahd36'] = [ahd_mean_36, ahd_std_36]
    result_dict['ahd25'] = [ahd_mean_25, ahd_std_25]
    result_dict['ahd_all_std'] = ahd_std_all
    result_dict['ahd_std_2356'] = ahd_std_2536

    result_dict['msd14'] = [msd_mean_14, msd_std_14]
    result_dict['msd36'] = [msd_mean_36, msd_std_36]
    result_dict['msd25'] = [msd_mean_25, msd_std_25]
    result_dict['msd_all_std'] = msd_std_all
    result_dict['msd_std_2356'] = msd_std_2536
    ###############################################################################
    ###############################################################################
    file = open('dict_for_ahdmsd_pddcav4_woboth.csv', 'w', encoding='utf-8', newline='')
    ###############################################################################
    ###############################################################################
...
if __name__ == "__main__":
    ...
    snapshot_final = '/home/pc/diskB/liTianLin/doubleTU/checkpoints/TU_pddcav1/TU2/epoch_80.pth'
    snapshot_wogan = '/home/pc/diskB/liTianLin/doubleTU/checkpoints/TU_pddcav3/TU2/epoch_80.pth'
    snapshot_wocl = '/home/pc/diskB/liTianLin/doubleTU/checkpoints/TU_pddcav2/TU2/epoch_80.pth'
    snapshot_woboth = '/home/pc/diskB/liTianLin/doubleTU/checkpoints/TU_pddcav4/TU2/epoch_80.pth'
    snapshot_baseline = '/home/pc/diskB/liTianLin/trans-unet/model/TU_pddca_syn36224/TU_pretrain_R50-ViT-B_16_skip3_epo150_bs8_224/epoch_149.pth'


    pred_final = '/home/pc/diskB/liTianLin/doubleTU/predictions/ABmixed_pddcav1/epoch_80'
    pred_wogan = '/home/pc/diskB/liTianLin/doubleTU/predictions/ABmixed_pddcav3/epoch_80'
    pred_wocl = '/home/pc/diskB/liTianLin/doubleTU/predictions/ABmixed_pddcav2/epoch_80'
    pred_woboth = '/home/pc/diskB/liTianLin/doubleTU/predictions/ABmixed_pddcav4/epoch_80'
    pred_baseline = '/home/pc/diskB/liTianLin/trans-unet/predictions/TU_pddca_syn36224/TU_pretrain_R50-ViT-B_16_skip3_epo150_bs8_224/'
    pred_aftgan = '/home/pc/diskB/liTianLin/trans-unet/TransUNet/predictions_pddcav1/TU_pddca_syn36224/TU_pretrain_R50-ViT-B_16_skip3_epo150_bs8_224'
    pred_full = '/home/pc/diskB/liTianLin/trans-unet/predictions/TU_pddca_trainset36224/TU_pretrain_R50-ViT-B_16_skip3_epo150_bs24_224/'
    #########################
    #########################
    ...
    ##########################
    ##########################
    predict_path = pred_final
    ##########################
    ##########################

    new_inference(args, net, predict_path, gt_dir='/home/pc/diskB/liTianLin/doubleTU/datasets/pddca_testset9/Seg')
