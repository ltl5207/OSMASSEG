...
parser.add_argument('--dataset', type=str, default='pddca_testset9', help='experiment_name')
...
def calculate_confusion_matrix_dice(args, pred_dir, label_dir, num_classes, net):
    ...
    stdmean6 = np.std(np.mean(dicematrix[:, 1:], axis=1))
    stdmean4 = np.std(np.mean(dicematrix[:, [2,3,5,6]], axis=1))
    return hist, per_class_dice(hist), dice_groupby_individual, stdlist3, stdlist6
...
def inference(args, model, test_save_path=None, gtdir=''):
    ...
    hist_matrix,dice,hist_matrix_groupby_individual,stdlist3, stdlist6 = calculate_confusion_matrix_dice(args, test_save_path, gtdir, 7, model)
    ...
    return dice[1:args.num_classes], performance, stdlist3, stdlist6
...
if __name__ == "__main__":
    ...
    # 这个肯定是每次都要改的
    args.exp = 'ABmixed_pddcav1'

    snapshot_path = './checkpoints/TU_pddcav4/TU2'
    snapshot_path_wo_epnum = './checkpoints/TU_pddcav4/TU2/epoch_'
    snapshot_suffix = '.pth'
    ...
    for _, epnum in enumerate(tqdm(epnum_list)):
        ...
        #要存，看结果
        if args.is_savenii:
            args.test_save_dir = '/home/pc/diskB/liTianLin/doubleTU/predictions'
            test_save_path = os.path.join(args.test_save_dir, args.exp, snapshot_name)
            os.makedirs(test_save_path, exist_ok=True)
        else:
            test_save_path = None

        dice, performance, stdlist3, stdlist6 = inference(args, net, test_save_path, gtdir='/home/pc/diskB/liTianLin/doubleTU/datasets/pddca_testset9/Seg')

        result_dict['{}'.format(epnum)] = dice,performance, stdlist3, stdlist6
    dict2csv(result_dict, './performance_dict_pddcav1_1.csv')
