...
class CUTModel(BaseModel):
    ...
    def compute_G_loss(self):
        ...
        #这一行也能跳到本体，只要在模型想断的那行打上断点，以前是f8stepover到不了所以才添了下面两行forward
        ...
        if self.opt.dce_idt:
            ...
            #rB到idtB用一组新的随机采样的id吧，因为pseudo标签不能给它用，它也没啥用，不用生成伪标签的增强采样点
            #先这样试一下
        ...
        self.loss_G = self.loss_G_GAN + loss_HDCE_both + self.loss_SRC #+ 0.01*self.loss_SRC_L
        return self.loss_G
