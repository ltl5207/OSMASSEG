...
class SRC_Loss(nn.Module):
    ...
    def forward(self, feat_q, feat_k, only_weight=False, epoch=None):
        ...
        diagonal = torch.eye(feat_k_v.shape[1], device=feat_k_v.device, dtype=self.mask_dtype)[None, :, :]
        HDCE_gamma = self.opt.HDCE_gamma
        if self.opt.use_curriculum:
            HDCE_gamma = HDCE_gamma + (self.opt.HDCE_gamma_min - HDCE_gamma) * (epoch) / self.opt.max_epochs # (self.opt.n_epochs + self.opt.n_epochs_decay)
