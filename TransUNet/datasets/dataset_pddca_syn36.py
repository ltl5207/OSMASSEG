...
class pddca_syn36_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.split == "train":
            ...
            # 正常测的时候要切回下面这个赋值
            #data_path = os.path.join(self.data_dir, 'JPEG', test_slice_name + '.jpg')
            #如果是gan之后的图像需要做这一步，其他的正常单通道灰度图做这一步也没影响
            im = Image.open(data_path).convert('L')#.resize([512,509])
            #im = Image.open(data_path)
            image = np.array(im)
            image_arr = image.astype(np.float) / float(255)
            label = Image.open(os.path.join('/home/pc/diskB/liTianLin/trans-unet/data/pddca_testset9/Seg', test_slice_name + '.png'))
            label_arr = np.array(label)
            ...
            label = Image.open(
                os.path.join('/home/pc/diskB/liTianLin/trans-unet/data/pddca_testset9/Seg', test_slice_name + '.png'))
            label_arr = np.array(label)
